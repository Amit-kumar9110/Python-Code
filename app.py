...
def extract_text_from_pdf():
    ...
    if uploaded_file:
        ...
        except PyPDF2.errors.PdfReadError as e:  # Catch specific PDF reading errors
...
def model_llm():
    return ChatGroq(
        ...
        temperature=0,
        groq_api_key="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"  # Use the API key from environment variables
    )
...
def query_pdf_content(query):
    if "uploaded_pdf_text" not in st.session_state or not st.session_state.uploaded_pdf_text:
        ...
        return None  # Return None to indicate no response
    ...
    try:
        ...
        return "An error occurred. Please check the logs."  # Return a message
...
def chat_ui():
    ...
    if st.session_state.active_conversation:
        ...
        for msg in conversation["messages"]:
            st.markdown(f"**You:** {msg['query']} ({msg['timestamp'].strftime('%H:%M')})")
